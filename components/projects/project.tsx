      ...
      <div className="w-full flex flex-col justify-center items-center lg:items-start ml-0 xl:ml-28 mr-0 lg:mr-20" data-aos="fade-left">
        ...
        <div className="w-full flex flex-row flex-wrap gap-x-4 justify-center md:justify-start items-center">
          {project.toolsUsed?.map((tool:string, index:number) => {
            return (<div key={index} className={`bg-gray-700 hover:bg-primary py-2 px-8 mt-8 text-white text-lg cursor-default rounded-md transform hover:scale-105 transition ease-in`}>{tool}</div>)
          })}
        </div>
      </div>
    </div>
